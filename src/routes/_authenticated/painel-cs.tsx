...
import { HeartPulse, RefreshCw, AlertTriangle, Hourglass } from "lucide-react";
...
function PainelCsPage() {
  ...
  const carregar = useCallback(async () => {
    const { data } = await supabase
      ...
      .select("pipefy_card_id,titulo,fase_atual,fase_atual_ordem,entrou_fase_atual_em,criado_em,concluido,fases_history")
      .limit(5000);
  ...
  const sync = useMutation({
    ...
    onSuccess: async (res) => {
      await carregar();
      toast.success(`Painel de CS atualizado do Pipefy: ${res.total} card(s).`);
    },
    onError: (e: unknown) => {
  ...
  const ativos = useMemo(() => rows.filter((r) => !r.concluido), [rows]);
  const concluidos = useMemo(() => rows.filter((r) => r.concluido), [rows]);
    ...
    <div className="space-y-4 p-4 md:p-6">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">Painel de CS — Onboarding</h1>
            <p className="text-sm text-muted-foreground">
              Visão executiva do pipe Pipefy "Onboarding Cliente" — funil, gargalos e tempo de ciclo do onboarding pós-venda
            </p>
          </div>
        </div>
        <Button
